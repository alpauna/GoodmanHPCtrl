...
- **Live dashboard** — Real-time dashboard at `/dashboard` with state banner, protection status pills (startup lockout countdown, short cycle, RV Pass/Fail with inline clear button, high suction temp), input/output grid, temperatures, and reboot button. Output indicators include inline status pills:
