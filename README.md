...
- **Live dashboard** — Real-time dashboard at `/dashboard` with state banner, protection status pills (startup lockout countdown, short cycle, RV fail with inline clear button, high suction temp), input/output grid, temperatures, and reboot button. Output indicators include inline status pills:
