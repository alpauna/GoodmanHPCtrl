...
- **Live dashboard** — Real-time dashboard at `/dashboard` with state banner, protection status pills with dynamic labels (LPS No Fault/Fault, Temp Ok/Low Temp, Comp Ok/OT, RV Pass/Fail with inline clear button, high suction temp), input/output grid, temperatures, and reboot button. Output indicators include inline status pills:
